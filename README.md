...
The system is designed to support NICUs and resource‑constrained hospitals by offering a low‑cost alternative to high‑end commercial monitors.
...
**Apnea Detection & Alerts**
  Instant visual + sound alerts when abnormal breathing is detected (e.g., breathing rate below threshold).

**Live Dashboard**
  Clean UI displaying breathing values, status, and system overview.

**Data Visualization**
  Graphs (e.g., trend plots) for analyzing breathing patterns over time.
